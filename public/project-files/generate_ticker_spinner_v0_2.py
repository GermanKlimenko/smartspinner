#!/usr/bin/env python3
"""Parametric mechanical envelope for Ticker Spinner v0.2 (12-pixel tip).
...
def export_cad(bottom: cq.Workplane, top_a: cq.Workplane,
    ...
    parts = {
        "ticker_spinner_v0_2_bottom": bottom,
        "ticker_spinner_v0_2_top": top_print,
        "ticker_spinner_v0_2_diffusers": diff_print,
        "ticker_spinner_v0_2_pcb_envelope": pcb.translate((0, 0, -P["pcb_z_bottom"])),
        "ticker_spinner_v0_2_tip_led_board_envelope": make_tip_board_flat(),
    }
    ...
    exporters.export(assembly, str(OUT / "ticker_spinner_v0_2_assembly.step"))
...
def export_dxf() -> None:
    ...
    m.add_text("Ticker Spinner v0.2 PCB mechanical template - mm - verify footprints before routing",
               height=1.2, dxfattribs={"layer": "NOTES", "insert": (-48, -55)})
    doc.saveas(OUT / "ticker_spinner_v0_2_pcb_outline.dxf")
    ...
    tm.add_text("VERTICAL TIP LED BOARD - 3 IDENTICAL - 7.0 x 13.6 x 0.6 mm - 12x WS2812B-0909",
                height=.8, dxfattribs={"layer":"NOTES", "insert":(-3.5,-6.3)})
    tip.saveas(OUT / "ticker_spinner_v0_2_tip_led_board.dxf")
...
def export_dimension_png() -> Path:
    ...
    fig.suptitle("TICKER SPINNER v0.2 — DIMENSIONAL REFERENCE (mm)", fontsize=16, weight="bold")
    ...
    side.set_xlim(-58, 65); side.set_ylim(-5, 21); side.set_aspect("auto"); side.axis("off")
    ...
    fig.text(0.52, 0.03,
             ...
             "Side windows: 12/arm. Three identical battery cavities maintain 120° mass symmetry.",
             ha="center", fontsize=9)
    path = OUT / "ticker_spinner_v0_2_dimension_drawing.png"
...
def export_pdf() -> None:
    pdf = OUT / "ticker_spinner_v0_2_engineering_drawing.pdf"
    ...
    story = [Paragraph("Ticker Spinner v0.2 — Engineering Reference", styles["Title"]), Spacer(1, 4*mm)]
    data = [
        ["Item", "Nominal", "Design intent / tolerance"],
        ["Overall envelope", "Ø100 × 16 mm", "Three identical arms at 120°"],
        ...
        ["Side LEDs", "36 total; 12/tip", "Addressable 0909 RGB, 1.05 mm pitch"],
    ...
    story += [table, Spacer(1, 5*mm),
              Paragraph("Critical v0.2 notes", styles["Heading2"]),
...
def _title_block(fig, sheet: str, title: str) -> None:
    fig.text(0.04, 0.965, "TICKER SPINNER v0.2", fontsize=17, weight="bold", va="top")
...
def export_multisheet_pdf() -> None:
    ...
    pdf_path = OUT / "ticker_spinner_v0_2_engineering_drawing.pdf"
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        conn.annotate("MAIN 3-ARM PCB", xy=(-4, 7), xytext=(-8, 18),
                      arrowprops=dict(arrowstyle="->"), fontsize=8, ha="center")
...
def validate_stl() -> dict:
    report = {}
    for path in sorted(OUT.glob("ticker_spinner_v0_2_*.stl")):
    ...
    (OUT / "ticker_spinner_v0_2_validation.json").write_text(
        json.dumps(report, indent=2), encoding="utf-8")
    return report


def write_readme(validation: dict) -> None:
    text = f"""# Ticker Spinner v0.2 - 12-pixel cylindrical ticker
...
```
"""
    (OUT / "README_Ticker_Spinner_v0_2.md").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    shutil.copy2(__file__, OUT / "generate_ticker_spinner_v0_2.py")
