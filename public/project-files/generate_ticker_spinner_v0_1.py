#!/usr/bin/env python3
"""Parametric mechanical envelope for Ticker Spinner v0.1.
...
P = {
    ...
    "bearing_width": 7.0,
    "bearing_z_bottom": 3.0,
    "pcb_thickness": 1.0,
    "pcb_z_bottom": 5.0,
    "pcb_tip_radius": 48.0,
    "pcb_center_hole": 23.2,
    "led_count_top_per_arm": 15,
    "led_radial_start": 14.8,
    "led_pitch": 2.2,
    "top_window_width": 3.0,
    "top_window_r0": 13.4,
    "top_window_r1": 46.3,
    "side_led_count_per_arm": 6,
    "side_led_z_start": 2.25,
    "side_led_z_pitch": 1.70,
    "side_window_width": 2.40,
    "side_window_height": 1.25,
    "tip_board_width": 7.0,
    "tip_board_height": 9.6,
    "tip_board_thickness": 0.8,
    "screw_radius": 30.0,
...
def tri_body(height: float, z0: float = 0.0, inset: float = 0.0,
...
def screw_positions() -> list[tuple[float, float]]:
    return [polar(P["screw_radius"], a, P["screw_tangent_offset"]) for a in (0, 120, 240)]
...
def make_bottom() -> cq.Workplane:
    ...
    cavity = tri_body(h - P["shell_floor"] + 0.2,
    ...
    # Battery pockets below the PCB, repeated exactly every 120 degrees.
    for angle in (0, 120, 240):
    ...
    # Three self-tapping screw bosses and pilot holes.
    ...
    # Lower three windows of the vertical six-pixel tip column.
...
def make_top_assembled() -> cq.Workplane:
    ...
    cavity = tri_body(cavity_h + 0.1, z0 - 0.05, inset=1.55, hub_radius=16.4)
    ...
    # Continuous serviceable windows for 15 LEDs per arm.
    ...
    # Upper three windows of the vertical six-pixel tip column.
    z_centers = [P["side_led_z_start"] + i * P["side_led_z_pitch"]
                 for i in range(int(P["side_led_count_per_arm"]))]
    for angle in (0, 120, 240):
...
def make_pcb() -> cq.Workplane:
    ...
    board = tri_body(P["pcb_thickness"], P["pcb_z_bottom"], inset=2.0, hub_radius=17.0)
...
def make_tip_boards_assembled() -> cq.Workplane:
    board0 = (cq.Workplane("YZ", origin=(46.2, 0, 6.5))
...
def export_cad(bottom: cq.Workplane, top_a: cq.Workplane,
    ...
    parts = {
        "ticker_spinner_v0_1_bottom": bottom,
        "ticker_spinner_v0_1_top": top_print,
        "ticker_spinner_v0_1_diffusers": diff_print,
        "ticker_spinner_v0_1_pcb_envelope": pcb.translate((0, 0, -P["pcb_z_bottom"])),
        "ticker_spinner_v0_1_tip_led_board_envelope": make_tip_board_flat(),
    }
    ...
    exporters.export(assembly, str(OUT / "ticker_spinner_v0_1_assembly.step"))
...
def export_dxf() -> None:
    ...
    zones = [(0, "nRF52/BLE 12x9"), (120, "POWER 12x9"), (240, "CHARGE/USB PADS 12x9")]
    ...
    m.add_text("Ticker Spinner v0.1 PCB mechanical template - mm - verify footprints before routing",
               height=1.2, dxfattribs={"layer": "NOTES", "insert": (-48, -55)})
    doc.saveas(OUT / "ticker_spinner_v0_1_pcb_outline.dxf")

    # Separate vertical tip-board template: three identical copies are required.
    ...
    for i in range(int(P["side_led_count_per_arm"])):
        z = P["side_led_z_start"] + i * P["side_led_z_pitch"] - 6.5
        tm.add_lwpolyline([(-1.0,z-.6),(1.0,z-.6),(1.0,z+.6),(-1.0,z+.6)],
    ...
    tm.add_text("VERTICAL TIP LED BOARD - 3 IDENTICAL - 7.0 x 9.6 x 0.8 mm",
                height=.8, dxfattribs={"layer":"NOTES", "insert":(-3.5,-6.3)})
    tip.saveas(OUT / "ticker_spinner_v0_1_tip_led_board.dxf")
...
def export_dimension_png() -> Path:
    ...
    fig.suptitle("TICKER SPINNER v0.1 — DIMENSIONAL REFERENCE (mm)", fontsize=16, weight="bold")
    ...
    ax.text(-48, 48, "45 × top RGB\n15/arm, pitch 2.20\nR14.8…45.6", va="top", fontsize=10)
    ax.text(-48, -40, "3 × M2 screws\nR30, tangent +5.3", va="top", fontsize=10)
    ax.set_aspect("equal")
    ax.set_xlim(-58, 58); ax.set_ylim(-58, 58); ax.axis("off")
    ax.set_title("TOP VIEW")

    # Simplified section through arm 1.
    side.add_patch(Rectangle((-50, 0), 100, 13, facecolor="#e7eaee", edgecolor="black"))
    side.add_patch(Rectangle((-11, 3), 22, 7, facecolor="#b0bec5", edgecolor="#1565c0", lw=2))
    side.add_patch(Rectangle((-48, 5), 96, 1, facecolor="#2e7d32", edgecolor="black"))
    side.add_patch(Rectangle((14, 1.6), 20, 3.2, facecolor="#90caf9", edgecolor="black"))
    side.add_patch(Rectangle((-46.3, 11.5), 32.9, 1.5, facecolor="#fff59d", edgecolor="#f9a825"))
    side.annotate("", xy=(52, 0), xytext=(52, 13), arrowprops=dict(arrowstyle="<->"))
    side.text(54, 6.5, "13.0 assembled", rotation=90, va="center")
    side.annotate("", xy=(12.5, 3), xytext=(12.5, 10), arrowprops=dict(arrowstyle="<->", color="#1565c0"))
    side.text(14.5, 6.5, "7.0 bearing", va="center", color="#1565c0")
    side.annotate("PCB 1.0 @ Z=5.0", xy=(38, 5.5), xytext=(22, -1.2),
                  arrowprops=dict(arrowstyle="->"), ha="center", fontsize=9)
    side.annotate("LiPo envelope 20×11.5×3.2", xy=(24, 3.2), xytext=(22, -3.2),
                  arrowprops=dict(arrowstyle="->"), ha="center", fontsize=9)
    side.annotate("1.5 roof + diffuser", xy=(-30, 12.3), xytext=(-30, 16.0),
                  arrowprops=dict(arrowstyle="->"), ha="center", fontsize=9)
    side.annotate("608 bearing 22×7×8", xy=(0, 9.5), xytext=(-2, 16.0),
                  arrowprops=dict(arrowstyle="->", color="#1565c0"),
                  ha="center", fontsize=9, color="#1565c0")
    side.set_xlim(-58, 65); side.set_ylim(-5, 18); side.set_aspect("auto"); side.axis("off")
    ...
    fig.text(0.52, 0.03,
             ...
             "Side windows: 6/arm. Three identical battery cavities maintain 120° mass symmetry.",
             ha="center", fontsize=9)
    path = OUT / "ticker_spinner_v0_1_dimension_drawing.png"
...
def export_pdf() -> None:
    pdf = OUT / "ticker_spinner_v0_1_engineering_drawing.pdf"
    ...
    story = [Paragraph("Ticker Spinner v0.1 — Engineering Reference", styles["Title"]), Spacer(1, 4*mm)]
    data = [
        ["Item", "Nominal", "Design intent / tolerance"],
        ["Overall envelope", "Ø100 × 13 mm", "Three identical arms at 120°"],
        ["Bearing", "608: 22 × 7 × 8 mm", "Printed seat Ø21.85; make fit coupon first"],
        ["Housing split", "6.5 + 6.5 mm", "Bottom tray + removable top"],
        ["PCB", "1.0 mm, max Ø96 envelope", "DXF is mechanical template, not finished layout"],
        ["Top LEDs", "45 total; 15/arm", "2.0 mm package courtyard; 2.20 mm radial pitch"],
        ["Side LEDs", "18 total; 6/tip", "Side/edge emitting package; confirm against selected part"],
        ["Battery cavities", "3 × 20 × 11.5 × 3.2 mm", "Use matched protected cells or one cell + equal ballast"],
        ["Fasteners", "3 × M2", "Top clearance Ø2.3; lower self-tap pilot Ø1.7"],
    ...
    story += [table, Spacer(1, 5*mm),
              Paragraph("Critical v0.1 notes", styles["Heading2"]),
              Paragraph("The model reserves mass symmetrically, but the populated PCB must be dynamically balanced. "
                        "Populate the three battery positions with matched cells only when the power architecture supports it; "
                        "otherwise use one cell and equal non-conductive dummy masses. Keep antenna copper/metal keep-out per the "
                        "chosen nRF52 module. Confirm LED, USB/pogo and Hall footprints before routing. The top diffuser is a "
                        "single three-strip STL and may be printed in translucent PETG or replaced with cast clear resin.",
                        styles["BodyText"])]
    doc.build(story)
...
def _title_block(fig, sheet: str, title: str) -> None:
    fig.text(0.04, 0.965, "TICKER SPINNER v0.1", fontsize=17, weight="bold", va="top")
    fig.text(0.04, 0.935, title, fontsize=12, va="top")
    fig.text(0.69, 0.04, "UNITS mm | SCALE NTS | PROTOTYPE",
             fontsize=8, family="monospace")
    fig.text(0.04, 0.04, "Circumscribed envelope Ø100 | Assembly thickness 13 | Bearing 608",
             fontsize=8, family="monospace")
...
def export_multisheet_pdf() -> None:
    """Four-sheet orthographic/mechanical reference set."""
    pdf_path = OUT / "ticker_spinner_v0_1_engineering_drawing.pdf"
    housing = _outline_for_shape(tri_body(1.0))
    pcb_pts = pcb_outline_points()
    inner = _outline_for_shape(tri_body(1.0, inset=1.55, hub_radius=16.4))
    grey, edge = "#e9edf0", "#202428"

    with PdfPages(pdf_path) as pdf:
        # Sheet 1 - general arrangement, three distinct projections plus section.
        ...
        ax.text(-54, 48, "45 TOP RGB\n15 PER ARM\nPITCH 2.20", fontsize=9, va="top")

        side = fig.add_axes([0.65, 0.59, 0.31, 0.20])
        side.set_title("B. SIDE ELEVATION", loc="left", fontsize=12, weight="bold")
        side.add_patch(Rectangle((-39.375, 0), 78.75, 13, fc=grey, ec=edge, lw=1.4))
        side.plot([-39.375, 39.375], [6.5, 6.5], color="#555", lw=0.8, ls="--")
        side.annotate("", xy=(43, 0), xytext=(43, 13), arrowprops=dict(arrowstyle="<->"))
        side.text(45, 6.5, "13.0", rotation=90, va="center")
        ...
        side.set_xlim(-48, 50); side.set_ylim(-5, 17); side.axis("off")

        sec = fig.add_axes([0.65, 0.24, 0.31, 0.24])
        sec.set_title("C. SECTION A-A THROUGH ARM", loc="left", fontsize=12, weight="bold")
        sec.add_patch(Rectangle((-39.375, 0), 78.75, 13, fc=grey, ec=edge, lw=1.2))
        sec.add_patch(Rectangle((-11, 3), 22, 7, fc="#b0bec5", ec="#1565c0", lw=1.5))
        sec.add_patch(Rectangle((-37, 5), 74, 1, fc="#2e7d32", ec=edge))
        sec.add_patch(Rectangle((14, 1.6), 20, 3.2, fc="#90caf9", ec=edge))
        sec.add_patch(Rectangle((-35, 11.5), 28, 1.5, fc="#fff59d", ec="#f9a825"))
        sec.annotate("PCB 1.0", xy=(32, 5.5), xytext=(27, 15), arrowprops=dict(arrowstyle="->"), fontsize=8)
        sec.annotate("LiPo cavity", xy=(24, 3), xytext=(18, -3), arrowprops=dict(arrowstyle="->"), fontsize=8)
        sec.annotate("608: 22×7×8", xy=(0, 9), xytext=(-12, 15), arrowprops=dict(arrowstyle="->"), fontsize=8)
        sec.set_xlim(-48, 50); sec.set_ylim(-5, 18); sec.axis("off")
        fig.text(0.65, 0.14, "FASTENERS: 3 × M2\nSPLIT LINE: Z=6.5\nBEARING AXIAL POCKET: Z=3…10",
        ...
        ax.text(-53, -45, "3 IDENTICAL BATTERY POCKETS\n20.6 × 12.1 × 3.45 CUT\nCENTERS R27", fontsize=9)
        ...
        detail.add_patch(Rectangle((-19, 0), 38, 6.5, fc=grey, ec=edge))
        detail.add_patch(Rectangle((-11, 3), 22, 3.5, fc="#b0bec5", ec="#1565c0", lw=1.5))
        detail.add_patch(Rectangle((-9.75, 0), 19.5, 3, fc="white", ec=edge))
        detail.annotate("3.0 RETAINING LIP", xy=(10, 1.5), xytext=(22, 1.5), arrowprops=dict(arrowstyle="->"), fontsize=8)
        detail.annotate("PRESS-FIT WALL Ø21.85", xy=(11, 5), xytext=(22, 6), arrowprops=dict(arrowstyle="->"), fontsize=8)
        detail.set_xlim(-25, 50); detail.set_ylim(-2, 10); detail.axis("off")
        fig.text(0.67, 0.43, "M2 BOSS\n- boss Ø5.0\n- pilot Ø1.7\n- three positions at 120°\n- verify self-tapping screw type",
        ...
        ax1.text(-54, 49, "3 TOP WINDOWS\n32.9 × 3.0\nROUNDED ENDS", fontsize=9, va="top")
        ax1.text(-54, -48, "6 VERTICAL TIP WINDOWS / ARM\nSEE SHEET 5", fontsize=9)
        ...
        fig.text(0.38, 0.10, "DIFFUSER INSERT: 3 separate translucent strips in one STL | 32.5 × 2.6 × 1.25 nominal | XY clearance 0.20/side",
        ...
        for ai, angle in enumerate((0,120,240), start=1):
            for i in range(int(P["led_count_top_per_arm"])):
                ...
                corners=[polar(r+dr,angle,dt) for dr,dt in [(-1,-1),(1,-1),(1,1),(-1,1)]]
        ...
        zone_colors=["#90caf9","#ffcc80","#ce93d8"]
        for (angle,label),zc in zip([(0,"nRF52/BLE"),(120,"POWER"),(240,"CHARGE")],zone_colors):
        ...
        rows=[("Yellow","45 × top RGB, 2×2 courtyard"),("Blue tip line","vertical daughterboard interface"),("Blue zone","nRF52/BLE allocation"),("Orange zone","power regulation"),("Purple zone","charge/interface"),("Red H","Hall index sensor")]
        y=.94
        ...
        notes.text(.02,y-.33,"Three trim-weight areas are provided in DXF.\nPerform static and dynamic balance after population.",fontsize=9,linespacing=1.5)
        notes.text(.02,y-.47,"BATTERIES",weight="bold",fontsize=10)
        notes.text(.02,y-.53,"Use a designed protected multi-cell pack, or one\ncell plus two equal non-conductive dummy masses.",fontsize=9,linespacing=1.5)
        pdf.savefig(fig); plt.close(fig)
        ...
        end.add_patch(Rectangle((-7.5, 0), 15, 13, fc=grey, ec=edge, lw=1.6))
        end.plot([-7.5, 7.5], [6.5, 6.5], ls="--", lw=.8, color="#555")
        z_centers = [P["side_led_z_start"] + i * P["side_led_z_pitch"]
                     for i in range(int(P["side_led_count_per_arm"]))]
        ...
        end.annotate("", xy=(-9, 0), xytext=(-9, 13), arrowprops=dict(arrowstyle="<->"))
        end.text(-10.2, 6.5, "13.0", rotation=90, va="center", fontsize=8)
        end.annotate("", xy=(-7.5, -1.2), xytext=(7.5, -1.2), arrowprops=dict(arrowstyle="<->"))
        end.text(0, -2.2, "TIP WIDTH 15.0", ha="center", fontsize=8)
        end.text(-7.2, 6.8, "CASE SPLIT", fontsize=7)
        end.set_xlim(-12, 13); end.set_ylim(-3, 16); end.set_aspect("equal"); end.axis("off")

        conn = fig.add_axes([0.38, 0.50, 0.25, 0.31])
        conn.set_title("B. TIP CONNECTION SECTION", loc="left", fontsize=12, weight="bold")
        conn.add_patch(Rectangle((0, 0), 18, 13, fc=grey, ec=edge, lw=1.4))
        conn.add_patch(Rectangle((2.8, 1.7), .8, 9.6, fc="#00897b", ec=edge, lw=1.0))
        conn.add_patch(Rectangle((-12, 5), 15.2, 1, fc="#2e7d32", ec=edge, lw=1.0))
        for z in z_centers:
            conn.add_patch(Rectangle((2.9, z-.6), 3.1, 1.2, fc="#29b6f6", ec="#0277bd", lw=.5))
        conn.annotate("VERTICAL LED BOARD\n7.0 × 9.6 × 0.8", xy=(3.2, 9), xytext=(9, 15),
                      arrowprops=dict(arrowstyle="->"), fontsize=8, ha="center")
        conn.annotate("MAIN 3-ARM PCB", xy=(-4, 5.5), xytext=(-8, 15),
                      arrowprops=dict(arrowstyle="->"), fontsize=8, ha="center")
        conn.annotate("SOLDER TABS / FPC", xy=(3.1, 5.5), xytext=(10, -2),
                      arrowprops=dict(arrowstyle="->"), fontsize=8, ha="center")
        conn.set_xlim(-14, 22); conn.set_ylim(-3, 18); conn.axis("off")
        ...
        for angle, color in zip((0,120,240),("#ef5350","#42a5f5","#66bb6a")):
        ...
        for z in np.linspace(.7,7.8,6): unwrap.plot([0,314.16],[z,z],color="#37474f",lw=.4)
        ...
        for x,c in zip((0,104.72,209.44),("#ef5350","#42a5f5","#66bb6a")):
        ...
        unwrap.text(318,4.25,"6 pixels\nhigh",va="center",fontsize=8)
        unwrap.set_xlim(-8,330); unwrap.set_ylim(-3,11); unwrap.axis("off")
        fig.text(0.68, 0.42,
                 "FUNCTION\nEach LED height becomes one horizontal pixel row.\nTimed flashes create angular columns as the tip moves.\nThe three arms are phase-shifted by 120° in firmware.",
                 fontsize=9, linespacing=1.5)
        fig.text(0.37, 0.42,
                 "MECHANICAL CHANGE\nThe cylindrical ticker requires three small vertical\ndaughterboards. Six LEDs on the flat main PCB would\nproduce only one display row, not readable text.",
                 fontsize=9, linespacing=1.5)
        pdf.savefig(fig); plt.close(fig)
...
def validate_stl() -> dict:
    ...
    (OUT / "ticker_spinner_v0_1_validation.json").write_text(
        json.dumps(report, indent=2), encoding="utf-8")
    return report


def write_readme(validation: dict) -> None:
    text = f"""# Ticker Spinner v0.1
...
- Верх: 15 LED на луч, шаг {P['led_pitch']:.2f} мм, 45 всего; рассчитано на корпус около 2 × 2 мм.
- Торец: вертикальная колонка 6 LED на луч, 18 всего; три сменные tip-board 7 × 9,6 × 0,8 мм.
- Крепёж: 3 × M2, симметрия 120°.
- Карманы: 3 × LiPo-envelope {P['battery_length']:.0f} × {P['battery_width']:.1f} × {P['battery_height']:.1f} мм.
...
```
"""
    (OUT / "README_Ticker_Spinner_v0_1.md").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    shutil.copy2(__file__, OUT / "generate_ticker_spinner_v0_1.py")
